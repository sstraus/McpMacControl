...
type ActionResult struct {
	...
	Extra   []mcp.Content  `json:"-"` // additional MCP content (e.g. inline image)
}
...
func executeScreenshot(index int, action Action) ActionResult {
	...
	imgContent, err := imageResult(img, prefix, format, action.Quality)
	if err != nil {
		return ActionResult{
			Index:   index,
			Type:    "screenshot",
...
func formatResults(results []ActionResult) *mcp.CallToolResult {
	...
	// Fast path: no extra content, return single text block
	if !hasExtra {
	...
	// Mixed content: interleave text and extra content (images or file paths)
	...
	for _, r := range results {
		...
		content = append(content, r.Extra...)
	}
	if !allSuccess && len(results) > 0 {
